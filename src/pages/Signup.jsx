...
import { registerWithEmail, loginWithGoogle, logoutUser, auth, db, getRedirectResultAuth } from "../firebase";
...
const formatAuthError = (err) => {
  ...
  switch (code) {
    case "auth/email-already-in-use":
      return "This email is already registered! Please click 'Sign In' to log in.";
    ...
    case "auth/popup-blocked":
      return "Sign-up popup was blocked by your browser. Please allow popups or try again.";
    case "auth/popup-closed-by-user":
      return "Sign-up was cancelled because the popup window was closed.";
    case "auth/cancelled-popup-request":
      return "Sign-up request already in progress. Please wait a moment.";
    case "auth/network-request-failed":
      return "Network error. Please check your internet connection.";
    case "auth/account-exists-with-different-credential":
      return "An account already exists with the same email using a different sign-in method.";
    default:
      return msg.includes("Firebase:") ? msg.replace(/^Firebase:\s*/, "") : (msg || "Signup failed. Please try again.");
  }
};
...
export default function Signup() {
  ...
  const establishUserSession = async (userObj, fallbackName = "") => {
    ...
    try {
      ...
      if (!userData) {
        ...
        userData = {
          ...DEFAULT_USER,
          name: fallbackName.trim() || userObj?.displayName || (admin ? "Ashish Sharma" : "Athlete"),
          email: cleanEmail,
          plan: admin ? "pro" : "free",
          sex: "male",
          goal: "muscle",
    ...
    setMessage({ text: "Account ready! Launching your dashboard... 🚀", type: "success" });
    ...
    getRedirectResultAuth()
      .then(async (result) => {
        if (result && result.user) {
          ...
          await establishUserSession(result.user);
        }
      })
      .catch((err) => {
        console.error("Redirect signin error:", err);
        setMessage({ text: formatAuthError(err), type: "error" });
      })
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      localStorage.setItem("ashfitverse_signup_name", form.name.trim());
      await establishUserSession(cred.user, form.name.trim());
    } catch (err) {
  ...
  const handleSocialSignin = async (providerFn) => {
    ...
    try {
      ...
      if (cred?.user) {
        await establishUserSession(cred.user);
      }
    } catch (err) {
  ...
  const css = generateCSS(T, dark) + `
    ...
    .scard{position:relative;z-index:10;width:100%;max-width:1160px;height:min(880px, 92vh);
          ...
          <div className="sfp">
            ...
            <div className="sbdy">
              ...
              <form className="sflds" onSubmit={onSubmit}>
                ...
                <button type="submit" className="regbtn" disabled={loading || socialLoading}>
